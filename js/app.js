...
function renderDevicesTable() {
    ...
    Object.entries(devices).forEach(([id, device]) => {
        ...
        // Perangkat dianggap Online HANYA JIKA statusnya 'online' DAN lastSeen kurang dari 2 menit yang lalu
        const isOnline = (device.status === "online") && (now - lastSeenTime < twoMinutesInMs);
        // -------------------------------
...
function updateStats() {
    ...
    const now = Date.now();
    const twoMinutesInMs = 2 * 60 * 1000; // Batas waktu 2 menit

    // Hitung Online dengan logika yang sama: Status harus 'online' DAN waktu harus baru
    const online = Object.values(devices).filter(device => {
        const lastSeenTime = device.lastSeen || 0;
        return (device.status === "online") && (now - lastSeenTime < twoMinutesInMs);
    }).length;

    const offline = total - online;

    // Update tampilan angka ke layar
