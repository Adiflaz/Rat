...
function renderDeviceInfo(device) {
    ...
    // --- MULAI PERBAIKAN LOGIKA SINKRONISASI ---
    const now = Date.now();
    const lastSeenTime = device.lastSeen || 0;
    const threshold = 2 * 60 * 1000; // Batas toleransi 2 menit

    // Status dianggap Online jika teks di DB 'online' DAN lastSeen kurang dari 2 menit yang lalu
    const isOnline = (device.status === 'online') && (now - lastSeenTime < threshold);
